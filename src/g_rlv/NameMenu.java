...
public class NameMenu extends BasicGameState{
	...
	public void enter(GameContainer arg0, StateBasedGame game) throws SlickException {
		game.addState(new g_rlv.MainMenu());
		game.addState(new g_rlv.World());
		game.addState(new g_rlv.ScoreMenu(game));
		game.addState(new g_rlv.CreditsMenu());
	}
	...
	public void update(GameContainer arg0, StateBasedGame game, int arg2) throws SlickException {
		compt++;
		if(compt>15) {
			game.enterState(g_rlv.MainMenu.ID,new FadeOutTransition(),new FadeInTransition());
		}
		
		if (compt==2) {
			game.getState(g_rlv.MainMenu.ID).init(arg0, game);
			game.getState(g_rlv.World.ID).init(arg0, game);
			game.getState(g_rlv.ScoreMenu.ID).init(arg0, game);
			game.getState(g_rlv.CreditsMenu.ID).init(arg0, game);
		}
	}
