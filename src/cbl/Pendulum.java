...
public class Pendulum extends BasicGameState{
	...
	private float calculateTheta() {
		return (float) (initialAngle*Math.cos(speed*(double)(omega*World.getTimeInMillis()/1000.0)));
	}
	private float calculateThetaDot() {
		return (float) (initialAngle*-1*omega*speed*Math.sin(speed*(double)(omega*World.getTimeInMillis()/1000.0)));
	}
