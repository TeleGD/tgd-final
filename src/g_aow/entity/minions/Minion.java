...
public class Minion {
	...
	public Minion(int idOwner, int age, int type) {
		...
		if (idOwner != 0) {
			...
			xp = 50 * age * type;
			damage = (int) (World.damageDefault * age * type);
			HP = (int) (World.HPminion * age * type);
			price = (int) (World.priceMinion * age * type);
			try {
    ...
    public void takeDamage(int inflictedDamage) {
    	...
    	if (idOwner != 0) {
    		...
    		if (this.HP<=0) {
    			if (idOwner == 1) {
    				World.p2.addGold((int) price );
    				World.p2.augmenteXp((int) xp );
    				
    			}
    			else {
    				World.p1.addGold((int) price );
    				World.p1.augmenteXp((int) xp );
    			}
